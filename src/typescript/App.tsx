...
const conf = {
    ...
    _code: "29ca3fd5f73319d1de900d8725d4ad04ce1c72c6"
}
...
function App() {
    useEffect(() => {
        ...
        const url2 = `/playlists?client_id=${conf._client_id}&format=jsonpretty&access_token=${conf._code}`
        console.log(url2);
        // getJSON(url2).then(data => { console.log("data==>> ", data) });
        const get_token = `/oauth/grant?client_id=${conf._client_id}&client_secret=${conf._secret}&grant_type=authorization_code&code=${conf._code}`;
        postJSON(get_token, {})
    }, [])
